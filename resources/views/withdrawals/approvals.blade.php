...
@section('content')
    @foreach ($users as $user)
        @if ($user->withdrawals->where('approval_status', 0)->isNotEmpty())
        <div class="card w-100">
            <form action="{{ route('approve-selected-withdrawals') }}" method="post">
                ...
                <div class="card-header bg-white">
                    <div class="row">
                        <div class="col">
                            <h3 class="card-title mb-0">Pending Withdrawal Approval</h3>
                            <h4 class="mb-0">Agent: {{ $user->name }}</h4>
                        </div>
                        <div class="col text-right">
                            @can('approve', 'App\Models\User')
                            <a href="{{ route('approve-all-withdrawals','agent='.$user->id) }}" class="btn btn-success">Approve All</a>
                ...
                <div class="card-body collapse show">
                    <div class="table-responsive no-wrap">
                        <table class="table product-overview v-middle file_export">
                            ...
                            <tbody>
                                @foreach ($user->withdrawals->where('approval_status', 0)->sortBy('account_number') as $withdrawal)
                                <tr>
                                    <td>
                                        {{ $loop->iteration }}
                                    </td>
